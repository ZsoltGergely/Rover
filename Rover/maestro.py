...
class __Controller:
    ...
    # def write(self, chan, angle):
    #     self.setTarget(chan, utils.translate(angle, 0, 180, self.Mins[chan], self.Maxs[chan]))
    #
    # def read(self, chan):
    #     return utils.translate(self.getPosition(chan), self.Mins[chan], self.Maxs[chan], 0, 180)
...
class Servo:
    def __init__(self, chan, min_value=-90, max_value=90, min_pulse=1600, max_pulse=11000, default=0, accel=80):
        if min_pulse < 0:
        ...
        servos.setRange(chan, self.__min_pulse, self.__max_pulse)
        servos.setAccel(chan, accel)
        servos.setTarget(self.__chan, int((default - self.__min_value) / (self.__max_value - self.__min_value) * (
    ...
    def read(self):
        global servos
        return servos.getPosition(self.__chan)
